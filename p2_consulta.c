...
int busca_binaria_mais_proxima(Leitura *leituras, int num_leituras, long timestamp_alvo){
    ...
    if (indice_mais_proximo > 0){
        ...
        if (dif_anterior < menor_diferenca){
            ...
            indice_mais_proximo = indice_mais_proximo - 1;
        }
    }
...
int main(int argc, char *argv[]){
    ...
    while (fgets(linha, sizeof(linha), arquivo_sensor) != NULL){
        ...
        if (sscanf(linha, "%ld %16s", &temp_timestamp, temp_id_sensor) == 2){
            ...
            if (ptr_valor != NULL){
                ...
                printf("Aviso: Linha ignorada no arquivo %s devido a formato inválido (valor ausente): %s", nome_arquivo_sensor, linha);
            }
        } else {
            printf("Aviso: Linha ignorada no arquivo %s devido a formato inválido (timestamp ou ID_SENSOR ausente): %s", nome_arquivo_sensor, linha);
        }
    }
    fclose(arquivo_sensor);
    ...
    printf("  Data e Hora: %s (Timestamp: %ld)\n", buffer_data_hora, leituras[indice_encontrado].timestamp);
